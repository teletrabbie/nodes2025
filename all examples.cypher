...
// Show shortest path (same query after deleting the common labels)
MATCH p = shortestPath((a:Substance)-[*..5]-(b:Substance)) 
WHERE a.Code='L04AX06' AND b.Code ='L01FX15'
RETURN p
;
...
// Show data as table
MATCH (n:Substance) RETURN n.Code, n.DDD, n.application;
...
// Example 4: Indirect (calculated) relations
...
// Show graph (same query after creating "indirect" relationship)
MATCH (s:Substance)
OPTIONAL MATCH (s)<-[q:HAS_ATC]-(m:Medicament)-[r:HAS_PRICE]->(p:Package)
RETURN *
;
...
// Example 5: Multiple node "layers"
